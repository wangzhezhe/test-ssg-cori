...
static void update_group_file(void* group_data, ssg_member_id_t member_id,
...
int main(int argc, char** argv) {
  ...
  if (!g_join) {
    g_drc_credential =
        g_drc_credential == -1 ? setup_credentials() : g_drc_credential;
  ...
  if (g_drc_credential != -1) hii.na_init_info.auth_key = cookie_str.c_str();
  ...
  if (!g_join) {
    ...
    ssg_group_create_mpi(engine.get_margo_instance(), "mygroup", MPI_COMM_WORLD,
                         &group_config, nullptr, nullptr, &gid);
  } else {
  ...
  mona_instance_t mona =
      mona_init_thread(g_address.c_str(), NA_TRUE, &hii.na_init_info, NA_TRUE);
  ...
  tl::managed<tl::pool> managed_colza_pool =
      tl::pool::create(tl::pool::access::mpmc);
  for (int i = 0; i < g_num_threads; i++) {
    colza_xstreams.push_back(tl::xstream::create(
        tl::scheduler::predef::basic_wait, *managed_colza_pool));
  }
  tl::pool colza_pool;
  if (g_num_threads == 0) {
  ...
  ssg_group_add_membership_update_callback(gid, update_group_file,
  ...
  for (int step = 0; step < g_num_iterations; step++) {
    ...
    spdlog::trace("sim iteration {}: results success {} info {}", step,
                  results.success(), results.error());
    // get the addr used for current iteration
    // how to make sure every process has the same view then move to the next
    // iteration? how to make sure new added process start from a specific
    // iteration?
...
void parse_command_line(int argc, char** argv, int rank) {
  try {
    ...
    TCLAP::ValueArg<std::string> logLevel(
        ...
        "info", "string");
    TCLAP::ValueArg<std::string> ssgFile("s", "ssg-file", "SSG file name",
                                         false, "", "string");

    TCLAP::SwitchArg joinGroup(
        "j", "join", "Join an existing group rather than create it", false);
    TCLAP::ValueArg<unsigned> swimPeriod(
        "p", "swim-period-length", "Length of the SWIM period in milliseconds",
        false, 1000, "int");
    TCLAP::ValueArg<int64_t> drc("d", "drc-credential-id",
                                 "DRC credential ID, if already setup", false,
                                 -1, "int");
...
void update_group_file(void* group_data, ssg_member_id_t,
...
#ifdef USE_GNI
  if (g_address.find("gni") == std::string::npos) return -1;
...
uint32_t get_credential_cookie(int64_t credential_id) {
  ...
  if (credential_id < 0) return drc_cookie;
  if (g_address.find("gni") == std::string::npos) return drc_cookie;
